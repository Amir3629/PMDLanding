...
export default async function ResourceArticle({
  ...
  const item = resources.find(entry => entry.slug === slug);
  if (!item) notFound();
  return <><PageHero eyebrow={item.category} title={item.title} intro={item.intro} image={item.articleImage || item.image} compact actions={false} /><article className="resourceArticle"><div className="articleBody">{item.sections.map(([title, body]) => <section key={title}><h2>{title}</h2><p>{body}</p></section>)}<div className="articleCallout"><strong>İş akışı ölçülebilir hale getirin</strong><p>Bu kılavuzdaki iş akışı için, mevcut adımları kaydetmek, sorumlu rol, tamamlanma durumu, kaynak sistemleri ve bir temel metrik. Sonra bu sonucu geliştirebilecek veya açıklayabilecek en küçük PayMyDine kapsamı yapılandırın.</p></div></div></article><CTA /></>;
}
