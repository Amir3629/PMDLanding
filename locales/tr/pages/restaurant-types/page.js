...
const setups = [
  ...
  ['Büyük veya çok katlı işletmeler', 'Daha karmaşık takımlar ve alanlar için birden fazla servis alanı planı, özelleştirilmiş çalışma alanları, merkezi raporlama ve seçili entegrasyonları yapılandırın.', '/site-assets/extra/team-cafe-meeting.webp']
];

export default function RestaurantTypesPage() {
  return (
    <>
      <PageHero eyebrow="4 restoran işletme modeli" title="Rolleri, salonu, müşteri yolculuğunu ve ölçütleri mevcut servis modelinize göre yapılandırın." intro="Ürün alanları aynı kalır; ancak öncelikler, yetkiler ve iş akışları restoran türüne göre değişir. Gerçek servis yolculuğuyla başlayın ve yalnızca onu destekleyen kontrolleri seçin." image="/site-assets/custom/shared-restaurant-hero-16.webp" />
      ...
      <section className="section capabilitySection"><div className="container capabilityPanel"><div><span className="eyebrow">Yapılandırma kararları</span><h2>Modül seçmeden önce her işletme için altı konuyu netleştirin.</h2><p>Rolleri, salon yapısını, sipariş kanallarını, müşteri temas noktalarını, mevcut sistemleri ve sonucu değerlendirmek için kullanılan başlangıç ölçütlerini belgeleyin.</p></div><div className="capabilityList"><span>Rol sorumluluğu</span><span>Salon ve masa modeli</span><span>Salonda servis ve paket servis kanalları</span><span>Müşteri sipariş yolculuğu</span><span>POS ve ödeme sınırları</span><span>Başlangıç operasyon ölçütleri</span></div></div></section>
      <CTA title="Gerçek servis modelinizi PayMyDine’a eşleyin." body="Bu yapı için gereken sorumlu rolleri, ürün alanlarını, salon yapılandırmasını, entegrasyonları ve ölçütleri birlikte belirleyelim." />
    </>
  );
}
