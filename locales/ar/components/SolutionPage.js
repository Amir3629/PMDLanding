  ...
  return <>
      ...
      <section className="section storyFeatureSection">
        <div className="container">
          {page.story.map((item, index) => <article className={`storyFeature ${index % 2 ? 'reverse' : ''}`} key={item.title}>
              ...
              <div className="storyFeatureCopy"><span className="eyebrow">{page.storyEyebrow || page.eyebrow}</span><h2>{item.title}</h2><p>{item.body}</p><a className="textArrow" href="/ar/how-it-works">انظر التدفق 5 خطوات <Icon name="arrow" size={15} /></a></div>
            </article>)}
        </div>
      </section>
      ...
      <section className="section capabilitySection">
        <div className="container capabilityPanel">
          <div>
            <span className="eyebrow">{page.capabilityEyebrow || "ما يمكن أن يفعله الفريق"}</span>
            <h2>{page.capabilityTitle || "حافظ على الإجراءات والمقاييس المطلوبة في تدفق عمل واحد"}</h2>
