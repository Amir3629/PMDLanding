...
export default function SmoothMotion() {
  ...
  useLayoutEffect(() => {
    ...
    */
    ...
    nodes.forEach(el => {
      ...
      */
      ...
      const delay = mobile ? Math.min(n, 4) * 18 : Math.min(n, 6) * 45;
      el.style.setProperty('--pmd-stagger', `${delay}ms`);
    });
    ...
    */
    ...
    const enterNodes = [...document.querySelectorAll('.heroCopy > *, .capabilityStrip, .pageHeroCopy > *, .pageHeroImage')];
    enterNodes.forEach((el, i) => {
      el.classList.remove('pmd-enter-v2');
      void el.offsetWidth;
      const delay = mobile ? 35 + Math.min(i, 6) * 30 : 60 + Math.min(i, 8) * 55;
      el.style.setProperty('--pmd-enter-delay', `${delay}ms`);
    ...
    */
    ...
    document.querySelectorAll('.heroVisual, .heroImage, .pmdHeroSecondaryLayerV2, .pmdHeroSecondaryImageV2').forEach(el => {
      el.classList.remove('pmd-enter', 'pmd-enter-v2', 'pmd-reveal', 'pmd-reveal-v2', 'pmd-in-view', 'pmd-in-view-v2', 'pmd-soft-lift', 'pmd-soft-lift-v2');
      el.style.removeProperty('--pmd-enter-delay');
      el.style.removeProperty('--pmd-stagger');
    });
    document.querySelectorAll('.offerCard,.resourceCard,.setupCard,.statusCard,.demoShowcaseCard,.storyFeatureImage,.pageHeroImage').forEach(el => el.classList.add('pmd-soft-lift-v2'));
    const reveal = el => {
    ...
    */
    ...
    const initialBoundary = mobile ? window.innerHeight * 1.18 : window.innerHeight * 0.96;
    nodes.forEach(el => {
      const r = el.getBoundingClientRect();
      if (r.bottom > 0 && r.top < initialBoundary) {
        reveal(el);
      }
    });
    ...
    */
    ...
    const io = new IntersectionObserver(entries => {
      ...
      threshold: mobile ? 0.01 : 0,
      rootMargin: mobile ? '0px 0px 22% 0px' : '0px 0px -4% 0px'
    });
    nodes.forEach(el => {
      if (!el.classList.contains('pmd-in-view-v2')) {
        io.observe(el);
      }
    });
    ...
    */
    ...
    const safety = window.setTimeout(() => {
      document.querySelectorAll('.pmd-reveal-v2:not(.pmd-in-view-v2)').forEach(el => {
        const r = el.getBoundingClientRect();
        const boundary = mobile ? window.innerHeight * 1.55 : window.innerHeight * 1.35;
        if (r.top < boundary) {
          reveal(el);
        }
      });
